...
ls /home/daviaaze/Projects/pessoal/ai-workspace/memory/
cat /home/daviaaze/Projects/pessoal/ai-workspace/memory/project-patterns.md | rg -i "<keyword>"
cat /home/daviaaze/Projects/pessoal/ai-workspace/memory/learning-log.md | rg -i "<keyword>"

# Check runbooks for operational procedures that may relate
ls /home/daviaaze/Projects/pessoal/ai-workspace/Runbooks/
```
...
# Otherwise, find relevant code with the agent's available tools:
#   - grep / find_path for symbol and path discovery
#   - inspect callers/callees by grepping for the symbol across the repo
#   - or the `code_graph` tool if available (check impact_radius first)
...
- Analyze the codebase for relevant code and blast radius: use `grep`/`find_path` for symbol discovery, query callers/callees by searching for symbol references, or the `code_graph` tool (check `impact_radius`) if available.
