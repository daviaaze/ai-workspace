...
2. **Scope the review** — from the diff, list changed files and find their direct callers/callees with `grep`/`find_path` (or the `code_graph` tool if available) to focus the review.
