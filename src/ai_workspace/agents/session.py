...
from ai_workspace.core.sessions import DEFAULT_COMPACTION_SETTINGS, SessionEntry, SessionStore
...
class PersistentAgentSession:
    ...
    async def _generate_summary(
        ...
        # Use agent_loop for summarization (faster than Crew)
        try:
            ...
            result = chat_sync(messages, provider="ollama", model="ministral-3:8b")
            return result or "Session summary not available."
    ...
    async def _run_agent_with_context(self, message: str, context: str) -> str:
        ...
        full_messages = [
            {
                "role": "system",
                "content": "You are a helpful coding assistant. Answer clearly." if not context
                else f"You are a helpful coding assistant. Use this context:\n\n{context[:15_000]}"
            },
            {"role": "user", "content": message},
        ]
        result = chat_sync(full_messages, provider="ollama", model=self.model or "qwen3:14b")
