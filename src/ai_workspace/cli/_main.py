...
def ask(
    ...
    if no_stream or provider != "ollama":
        ...
        with console.status(f"[cyan]Thinking ({model})...", spinner="dots"):
            try:
                response = chat_sync(messages, provider=provider, model=model, max_tokens=8192)
            except Exception as e:
                console.print(f"[red] Error: {e}[/]")
        ...
        try:
            response = chat_sync(messages, provider=provider, model=model, stream=True, on_token=print_token, max_tokens=8192)
        except Exception as e:
            console.print(f"\n[red] Error: {e}[/]")
            cost.budget.record_failure(
                provider=provider, model=model or "unknown",
...
# Trace and chat commands (extracted to cli._chat)
from ai_workspace.cli._chat import *  # noqa: F401, F403
